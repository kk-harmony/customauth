...
**Production** requires environment variables only ? see **[PRODUCTION.md](PRODUCTION.md)**.

**Fly.io** ? see **[FLY.md](FLY.md)** for Fly secrets and `*.fly.dev` deployment.
...
- `OAuthServer__SigningCertificatePath` ? path to the `.pfx` inside the container or host
- `OAuthServer__SigningCertificatePassword` ? export password

**Never commit** `.pfx`, `.key`, or passwords to git (`*.pfx` is in `.gitignore`).

#### Option A ? OpenSSL (self-signed or internal CA)
...
Configure the running app (or Fly secrets ? see [FLY.md](FLY.md)):
...
1. **App users** ? SQL scripts in `Migrations/Sql/` (applied on startup via `schema_migrations`).
2. **OpenIddict** ? EF Core store. On startup:

Migrations are applied by `docker compose` (`migrate` service) or `./scripts/run-database-migrations.sh` ? not at API startup.
...
> **Production:** See [PRODUCTION.md](PRODUCTION.md) ? env-only secrets, HTTPS issuer, signing certs, and migrations via `migrate` service.
...
### Example: create a BFF client for token exchange

Confidential clients with the `token_exchange` grant must declare which upstream API audiences they may request via `allowedAudiences`:
...
curl -s -X POST "$BASE/api/admin/clients" \
  ...
  -d '{
    "clientId": "my-bff",
    "displayName": "My BFF Service",
    "clientType": "confidential",
    "clientSecret": "change-me",
    "grantTypes": ["token_exchange"],
    "scopes": ["api"],
    "allowedAudiences": ["orders-api", "inventory-api"]
  }'
```
...
`allowedAudiences`: required for token-exchange clients; RFC 8693 `audience` values the client may request.
...
A BFF or backend service exchanges a **user** access token for an audience-scoped upstream token. The minted token carries the end-user `sub` (not the BFF client ID) and an `act` claim identifying the requesting service.

1. Obtain a user access token (authorization code + PKCE above). Set `USER_TOKEN` to that access token.
2. Exchange it for an upstream token targeting a configured audience:
...
3. Verify the delegated token via introspection (`sub` = user ID, `aud` = audience, `act` = BFF client ID):

```bash
curl -s -X POST "$BASE/connect/introspect" \
  -d "token=$UPSTREAM_TOKEN" \
  -d "client_id=m2m-client" \
  -d "client_secret=m2m-secret-change-in-production" | jq '{sub, aud, act, scope}'
```

Client-credentials tokens cannot be used as `subject_token`; the exchange requires a token whose subject is a registered end user.
...
1. **Docker Desktop** ? Testcontainers starts `postgres:16-alpine` automatically
2. **`docker compose up -d postgres`** from main `docker-compose.yml` (same port/database)
3. **`CUSTOMOAUTH_TEST_CONNECTION`** ? custom connection string (use **single quotes** in bash if the password contains `$`)
...
- Development uses auto-generated signing certificates; **Production requires a PFX** ? see [Create signing PFX for Production](#create-signing-pfx-for-production).
