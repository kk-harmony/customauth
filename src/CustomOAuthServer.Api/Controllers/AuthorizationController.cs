    ...
    ILogger<AuthorizationController> logger) : ControllerBase
{
    ...
    public async Task<IActionResult> Authorize(CancellationToken cancellationToken)
    {
        ...
        if (!result.Succeeded)
        {
            // Browser OAuth flows need a 302 to the login page. [ApiController] Challenge returns 401 + Location,
            // which leaves a blank page when the user opens /connect/authorize directly.
        ...
        identity.SetScopes(ScopeAuthorization.FilterScopesForUser(request.GetScopes(), user.Roles));
